...
EPUBJS.Render.Iframe.prototype.load = function(contents, url){
    ...
    //fix1
    try {
      var cont1 = contents.substring(contents.indexOf("<body") + 1);
          cont1 = cont1.substring(cont1.indexOf(">") + 1);
          cont1 = rebuildDialogHTML(cont1.split("</body>")[0]);
      var cont2 = contents.split("<head>")[1].split("</head>")[0];
    } catch(e) {
      var cont1 = rebuildDialogHTML(contents);
      var cont2 = contents;
    }
...
EPUBJS.Unarchiver.prototype.getUrl = function(url, mime){
  ...
  if(!entry) {
    ...
    if (type == "jpeg" || type == "png" || type == "jpg") window.epubJsError1 = true;
		deferred.reject({
